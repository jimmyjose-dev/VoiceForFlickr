...
- (void)setOn:(BOOL)switchValue {
	if (on != switchValue) {
		...
		if (on) {
			...
			tmpFrame = offLabel.frame;
			tmpFrame.origin.x = slider.frame.origin.x + slider.frame.size.width - margin+20;
			offLabel.frame = tmpFrame;
...
offLabelBackgroundColor:(UIColor *)offLabelBackgroundColor
  ...
  offLabelTextColor:(UIColor *)offLabelTextColor
	...
	if (self = [super initWithFrame:frame]) {
		...
		slider.color = [UIColor colorWithPatternImage:[UIImage imageNamed:@"visibilty_knob.png.png"]];//sliderColor;
		...
		offLabel = [[UILabel alloc]
					initWithFrame:CGRectMake(slider.frame.size.width+20 - margin, yCord,
											 labelWidth, height)]; 
